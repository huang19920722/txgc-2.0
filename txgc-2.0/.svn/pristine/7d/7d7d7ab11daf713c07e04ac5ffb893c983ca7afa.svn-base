package com.yp.sys.service.menu.impl;
...
public class MenuServiceImpl extends BaseServiceImpl<Menu> implements IMenuService {
	...
	public List<TreeNodeBean> tree(String id,Long userId,String time) {
		...
		if(userId==0){
			hql=" from Menu t where  function=0 and recordStatus=? order by t.seq";
			menus = menuDao.find(hql,GlobalConstant.FLAG_Y);
		}else{
			hql=" from Menu t where FIND_IN_SET(id,queryMenusByUserId(?))>0 and function=0 and recordStatus=? order by t.seq";
			menus = menuDao.find(hql,userId,GlobalConstant.FLAG_Y);
		}
...
//		}
		...
		if (!"".equals(condition)) {
			condition = "and t.id in (" + condition + ")";
		} else {
			condition = " and t.id=-1";
		}
		// 查出当前用户所拥有的菜单
		hql = " from Menu t where function=0 " + condition+ " order by t.seq";
	...
	private String findMenuIdsStrByUserId(Long userId) {
		String sql1 = "select rm.SYMENU_ID from SYS_ROLE_TO_MENU rm where  rm.ROLE_ID in (select ur.SYROLE_ID from SYS_USER_TO_ROLE ur where ur.SYUSER_ID='"
				+ userId + "')";
		Long l = 0l;
		if (l.equals(userId)) {
			sql1 = "select id from sys_menu where recordStatus='Y'";
		}
		// 用户拥有的权限
	...
	private String findMenuIdsStrByRoleId(String roleId) {
		String sql = "select rm.SYMENU_ID from SYS_ROLE_TO_MENU rm where rm.ROLE_ID ='"
				+ roleId + "'";
	...
	public List<TreeNodeBean> treeAll(String userId) {
		//String hql1 = "from Menu t where recordStatus= '"+GlobalConstant.FLAG_Y+"'"+" order by t.seq";
		//List<Menu> menus = menuDao.find(hql1);
		String hql="";
		Long userIdL= GlobalContext.getCurrentUser().getId();
		List<Menu> menus=new ArrayList<Menu>();
		//List<MenuBean> treegrid=new ArrayList<MenuBean>();
		if(userIdL==0){
			hql="from Menu where recordStatus=? order by seq";
			menus= menuDao.find(hql,GlobalConstant.FLAG_Y);
		}else{
			String sql="select rm.SYMENU_ID from SYS_ROLE_TO_MENU rm where  rm.ROLE_ID in (select ur.SYROLE_ID from SYS_USER_TO_ROLE ur where ur.SYUSER_ID='"+userIdL+"')";
			...
			hql="from Menu t where t.recordStatus=? and t.id in("+idStr+") order by seq";
			menus=menuDao.find(hql,GlobalConstant.FLAG_Y);
	...
	public List<TreeNodeBean> findTreeByRole(String roleId) {
		...
		String hql = " from Menu t where t.symenu is null and recordStatus=? and FIND_IN_SET(id,queryMenusByRoleId(?))>0 ";
		...
		List<Menu> symenus = menuDao.find(hql,GlobalConstant.FLAG_Y,Long.valueOf(roleId));
		List<TreeNodeBean> tree = new ArrayList<TreeNodeBean>();
...
//	@Transactional(readOnly = true)
	public List<TreeNodeBean> buildTreeByRole(String roleId) {
		/*String hql = "from Menu t where recordStatus=? and FIND_IN_SET(id,queryMenusByRoleId(?))>0 order by t.seq";
		List<Menu> menus = menuDao.find(hql,GlobalConstant.FLAG_Y,Long.valueOf(roleId));*/
		String condition = findMenuIdsStrByRoleId(roleId);
		if (StringUtil.isBlank(condition)) {
			condition = "-1";
		}
		// 根据权限查询菜单
		String hql2 = "from Menu t where t.id in(" + condition
				+ ") order by t.seq";
	...
	public DataGridJson treegrid(Long userId) {
		...
		if(userId==0){
			...
			String sql="select rm.SYMENU_ID from SYS_ROLE_TO_MENU rm where  rm.ROLE_ID in (select ur.SYROLE_ID from SYS_USER_TO_ROLE ur where ur.SYUSER_ID='"+userId+"')";
			List<Object> list=baseDao.findBySql(sql);
			String idStr="";
			if(list!=null && list.size()>0 ){
				for(Object obj:list){
	...
	public List<Menu> findByParentId(String parentId,Long userId) {
		...
		if(userId==0){
			hql= "select id from Menu where pid=?  order by seq";
			menuIds=menuDao.find(hql,parentId,GlobalConstant.FLAG_Y);
		}else{
			hql= "select id from Menu where pid=? and recordStatus= ? and FIND_IN_SET(id,queryMenusByUserId(?))>0  order by seq";
			menuIds=menuDao.find(hql,parentId,GlobalConstant.FLAG_Y,userId);
		}
...
//	@CacheEvict(value = "syproAllMenuCache", allEntries = true)
	...
	public Map<String,Menu> treegrid() {
		String hql = "from Menu t order by t.seq where recordStatus=?";
		List<Menu> symenus = menuDao.find(hql,GlobalConstant.FLAG_Y);
...
//			menus=menuDao.find(hql,GlobalConstant.FLAG_Y);
//		}else{
//			hql= "from Menu where recordStatus= ? and FIND_IN_SET(id,queryMenusByUserId(?))>0  order by seq";
//			menus=menuDao.find(hql,GlobalConstant.FLAG_Y,id);
//		}
//		return  menus;
	}
	...
	public List<TreeNodeBean> findTree4Permissions(Long userId, String roleId) {
		...
		String hql="";
		if(userId==0){
			hql=" from Menu t where  recordStatus=? order by t.seq";
			userMenus = menuDao.find(hql,GlobalConstant.FLAG_Y);
		}else{
			hql=" from Menu t where FIND_IN_SET(id,queryMenusByUserId(?))>0 and recordStatus=? order by t.seq";
			userMenus = menuDao.find(hql,userId,GlobalConstant.FLAG_Y);
		}
		//角色拥有权限
		String hql2="from Menu where  recordStatus=? and FIND_IN_SET(id,queryMenusByRoleId(?))>0 ";
		List<Menu> roleMenus=menuDao.find(hql2,GlobalConstant.FLAG_Y,Long.valueOf(roleId));
		*/
		// 用户拥有的权限
		String condition = findMenuIdsStrByUserId(userId);
		if (!"".equals(condition)) {
...
//		}else{
//			hql=" from Menu t where FIND_IN_SET(id,queryMenusByUserId(?))>0  and recordStatus=? order by t.seq";
//			menus = menuDao.find(hql,id,GlobalConstant.FLAG_Y);
//		}
		// 存放当前用户所拥有的菜单
		String condition = findMenuIdsStrByUserId(id);
		if (!"".equals(condition)) {
	...
	@Override
	public List<ZtreeBean> loadMenus(String roleId, String pid, String url) {
	...
	//	List<Menu> menus=new ArrayList<Menu>();
		...
		// 创建菜单集合
		List<ZtreeBean> root = new ArrayList<ZtreeBean>();
		// 循环添加菜单到菜单集合
		if (null != menus && menus.size() > 0) {
			for (int i = 0; i < menus.size(); i++) {
				Menu menu = menus.get(i);
				ZtreeBean item = new ZtreeBean(menu.getText(), null);
		...
		if (null != root && root.size() > 0) {
			...
			for (int i = 0; i < root.size(); i++) {
				ZtreeBean ee = root.get(i);
				loadChildrenByPid(ee, url, roleId,idStr);
			}
		}
		return root;
	}
	 ...
	 * @param u
	 */
	private void loadChildrenByPid(ZtreeBean item, String url, String roleId,String idStr) {
		// 加载菜单节点
		...
		if (StringUtils.isNotBlank(roleId)) {
			...
			if (null != role && null != role.getMenus()
					&& role.getMenus().size() > 0) {
				for (int i = 0; i < role.getMenus().size(); i++) {
					menuIds += i == 0 ? "'" + role.getMenus().get(i).getId()
							+ "'" : ",'" + role.getMenus().get(i).getId() + "'";
				}
			}
		}
		...
		if (data != null && data.size() > 0) {
			if (item.getChildren() == null)
				item.setChildren(new ArrayList<ZtreeBean>());
			// 创建菜单节点
			for (int i = 0; i < data.size(); i++) {
				Menu entry = data.get(i);
				ZtreeBean addItem = new ZtreeBean(entry.getText(), null);
			...
			if (null != item.getChildren() && item.getChildren().size() > 0) {
				for (int i = 0; i < item.getChildren().size(); i++) {
					loadChildrenByPid(item.getChildren().get(i), url, roleId,idStr);
				}
			}
		}
