...
<script type="text/javascript" charset="UTF-8">
	...
	$(function(){
			...
			buttons : [ {
				...
				handler : function() {
					if(villageForm.form('validate')){
						...
						var villageUrl="${ctx}/params/vilageAdd";
						$.ajax({
							url:villageUrl, 
		...
		datagrid = $('#datagrid').datagrid({
			...
			nowrap : false,
			border : false,
			columns:dc.defineColumns('1479284829528'),
	...
	function village() {
		...
		if(rows.length > 0){
			for(var i=0; i<rows.length; i++){
				ids.push(rows[i].communityScrambler);
		...
		<div id="toolbar" class="search_table">
			<table>
				<tr>
					...
					<td>
						无线厂商设备：
						<!--<input name="wirelessEquipmentManufacturer" class="easyui-combobox" style="height:26px;"/>-->
				...
				<tr>
					<td>
						...
						-->
						<a class="easyui-linkbutton" onclick="dc.showDefineDialog('1479284829528')" href="javascript:void(0);">自定义列</a>
					</td>
				</tr>
			</table>
		</div>
